...
const semestres = [
  ...
  },
  {
    nombre: "Semestre 7",
    ramos: [
      { id: "adulto-iii", nombre: "Adulto III" }, // anual
...
// IDs especiales de urgencias (tipo de ramo, no por semestre)
const URGENCIAS_IDS = [
...
function manejarClickRamo() {
  ...
  // - En semestre 9 solo se puede aprobar una de las dos urgencias.
  if (URGENCIAS_IDS.includes(idRamo) && semestre === "9") {
    const otraUrgenciaYaAprobada = URGENCIAS_IDS.some(function (id) {
      return id !== idRamo && aprobados.has(id);
    });
    if (otraUrgenciaYaAprobada) {
      mostrarMensaje(
        "En el semestre 9 solo puedes aprobar una de las asignaturas de urgencias. " +
          "Si quieres cambiarla, desmarca primero la otra urgencia."
      );
      return;
    }
  }

  // Verificación de requisitos normales
...
function obtenerRequisitosFaltantes(idRamo) {
  ...
  return reqs.filter(function (idReq) {
    return !aprobados.has(idReq);
  });
}

/**
 * Actualiza visualmente qué ramos están bloqueados (no se pueden aprobar aún).
 */
function actualizarBloqueos() {
  ...
  elementosRamos.forEach(function (elem) {
    ...
    // Si ya está aprobado, no se marca como bloqueado
    ...
    // Regla extra de urgencias: en semestre 9, si una ya está aprobada, la otra se bloquea.
    if (URGENCIAS_IDS.includes(idRamo) && semestre === "9") {
      const otraUrgenciaYaAprobada = URGENCIAS_IDS.some(function (id) {
        return id !== idRamo && aprobados.has(id);
      });
    ...
    // Si pasa todos los chequeos, no bloqueado
    elem.classList.remove("bloqueado");
  });
}
